...
{
    if (EPWM_GetDutyIntFlag(EPWM0, 0))
    {
        /* Clear channel 0 period interrupt flag */
        EPWM_ClearDutyIntFlag(EPWM0, 0);

        EPWM_DisableOutput(EPWM0, EPWM_CH_0_MASK);            
        EPWM_Stop(EPWM0, BIT0);
        EPWM_ForceStop(EPWM0, BIT0);    
    }
}

void pulse_pwm_trigger(void)
{
    EPWM_ConfigOutputChannel(EPWM0, 0, 3800 , 95);    
...
{
    SYS->GPA_MFPL = (SYS->GPA_MFPL & (~SYS_GPA_MFPL_PA5MFP_Msk)) | EPWM0_CH0_PA5;
    GPIO_SetSlewCtl(PA, BIT5 , GPIO_SLEWCTL_FAST);

    EPWM_EnableDutyInt(EPWM0, 0,EPWM_DUTY_INT_UP_COUNT_MATCH_CMP);
